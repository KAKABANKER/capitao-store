...
async function criarPixPlumify(dadosCliente, total, itens, host) {
    ...
    try {
        ...
        if (data && data.pix_qr_code) {
            ...
            return { pix_qr_code: data.qr_code, id: data.id };
        } else {
            return null;
        }
    } catch (error) {
...
// GERAR PIX NA PLUMIFY (COM QR CODE)
app.post('/api/gerar-pix', async (req, res) => {
    const { cliente, total, itens } = req.body;
    const protocol = req.headers['x-forwarded-proto'] || 'https';
    const host = req.headers['host'];
    
    console.log('🟢 Gerando PIX na Plumify para:', cliente.cliente_nome, 'Total:', total);
    
    const result = await criarPixPlumify(cliente, total, itens, `${protocol}://${host}`);
    
    if (result && result.pix_qr_code) {
        console.log('✅ PIX gerado com sucesso! QR Code recebido');
        res.json({ 
            success: true, 
        ...
        console.log('❌ Falha ao gerar PIX na Plumify');
        res.json({ 
            success: false, 
            error: 'Erro ao gerar PIX na Plumify' 
        });
    }
});
...
app.post('/api/webhook/plumify', async (req, res) => {
    console.log('📡 Webhook Plumify recebido:', req.body);
    res.json({ success: true });
});
